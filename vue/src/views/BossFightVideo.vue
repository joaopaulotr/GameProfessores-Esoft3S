<template>  <div class="video-container" @click="skipVideo">
    <video
      ...
      src="/src/assets/videos/videoBatalha.mp4"
      autoplay
    ></video>
...
const onVideoEnd = () => {
  // Redireciona para a batalha mantendo o ID do boss
  if (bgMusic) bgMusic.play(); // Retorna a música
  router.push({
    path: '/battle',
    query: route.query
  });
...
onMounted(() => {
  ...
  if (videoPlayer.value) {
    videoPlayer.value.play().catch(error => {
      ...
      // Em caso de erro, vai direto para a batalha
      if (bgMusic) bgMusic.play(); // Retorna a música em caso de erro
      router.push({
        path: '/battle',
        query: route.query
      });
...
// Garante que a música volte quando o componente for destruído
onUnmounted(() => {
  if (bgMusic) {
    bgMusic.play();
  }
